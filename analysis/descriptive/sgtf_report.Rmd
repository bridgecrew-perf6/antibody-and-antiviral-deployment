...
print(tbls1$table_body$stat_0)
print(str_extract(string = tbls1$table_body$stat_0, pattern = "[0-9]+"))
print(str_extract(string = tbls1$table_body$stat_0, pattern = "[0-9]+") > 0 &
  str_extract(string = tbls1$table_body$stat_0, pattern = "[0-9]+") < threshold)

tbls1$table_body$stat_0 = ifelse(str_extract(string = tbls1$table_body$stat_0, pattern = "[0-9]+") > 0 &
  str_extract(string = tbls1$table_body$stat_0, pattern = "[0-9]+") < threshold, "<8", tbls1$table_body$stat_0)
...
print(tbls2$table_body$stat_0)
print(str_extract(string = tbls2$table_body$stat_0, pattern = "[0-9]+"))
print(str_extract(string = tbls2$table_body$stat_0, pattern = "[0-9]+") > 0 &
  str_extract(string = tbls2$table_body$stat_0, pattern = "[0-9]+") < threshold)

tbls2$table_body$stat_0 = ifelse(str_extract(string = tbls2$table_body$stat_0, pattern = "[0-9]+") > 0 &
  str_extract(string = tbls2$table_body$stat_0, pattern = "[0-9]+") < threshold, "<8", tbls2$table_body$stat_0)

print(tbls2$table_body$stat_0)

tbl_merge(list(tbls1, tbls2), tab_spanner = c("**Earliest Specimen Dataset**", "**All Tests Dataset**"))
```
...
#### Results
Between 1st March 2020 and 4th June 2022, of all (first weekly) positive SARS-CoV-2 tests available in OpenSAFELY from Second Generation Surveillance System (SGSS) data which captures routine laboratory surveillance data on infectious diseases across England, about `r 100 - as.numeric(totals[4,3])`% had SGTF data available;
